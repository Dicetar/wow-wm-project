...
def test_live_pinned_b00_inserts_quest_add_for_quest_783():
    ...
    assert "\"quest_id\":783" in quest_add_sqls[0]
    assert "5407" in quest_add_sqls[0]


def test_live_open_b01_after_approval_inserts_quest_add_for_quest_15():
    rt, rc, _ = _live_runtime()
    ...
    op.approve_next()
    quest_add_sqls = [s for s in rc.executed if "'quest_add'" in s]
    assert any("\"quest_id\":15" in s for s in quest_add_sqls)
