...
class BoneboundRuntimeStaticTests(unittest.TestCase):
    ...
    def test_alpha_echo_damage_is_runtime_hooked_not_visible_field_copy(self) -> None:
        ...
        self.assertIn("void MatchBoneboundEchoMovementSpeed(Pet* alphaPet, TempSummon* echo, float speedMultiplier = 1.0f)", runtime)
        self.assertIn("float multiplier = std::isfinite(speedMultiplier) && speedMultiplier > 0.0f", runtime)
    ...
    def test_alpha_echoes_reacquire_alpha_victim_when_follow_motion_sticks(self) -> None:
        ...
        self.assertIn("victim = SelectBoneboundEchoSeekTarget(owner, echo, huntRadius, diff);", runtime)
    ...
    def test_priest_echo_variant_is_role_based_and_visible_spell_casting(self) -> None:
        ...
        self.assertIn("Unit* SelectBoneboundPriestEnemyTarget(Creature* priestEcho, Player* owner, Pet* alphaPet, uint32 ownerGuid, WmSpells::BoneboundBehaviorConfig const& config, uint32 diff)", runtime)
        self.assertIn("Unit* sought = SelectBoneboundEchoSeekTarget(owner, priestEcho, ResolveBoneboundEchoHuntRadius(ownerGuid, runtimeConfig), diff);", runtime)
        self.assertIn("enemy && IsBoneboundEchoHuntMode(state.ownerGuid)", runtime)
        self.assertIn("float readyRange = std::max(5.0f, castRange - 1.0f);", runtime)
        self.assertIn("priestEcho->GetMotionMaster()->MoveChase(enemy, chaseStopDistance);", runtime)
        ...
        self.assertIn("float priestEchoMovementSpeedMultiplier = 1.5f;", header)
        self.assertIn('ExtractJsonFloat(configJson, "priest_echo_movement_speed_multiplier")', runtime)
