...
- `scripts/bridge_lab/Start-BridgeLabAutoBounty.ps1` is the repo-owned BridgeLab operator lane for the opt-in dynamic path: it stops the current native watcher, deactivates existing `reactive_bounty:*` rules for the scoped player by default, then relaunches `wm.events.watch` with `WM_REACTIVE_AUTO_BOUNTY_ENABLED=1` and `--arm-from-end`
...
- Bonebound Alpha native lane is `WORKING` at repo/build level on shell `940001`: bridge-lab SQL now binds `summon_bonebound_alpha_v3`, `spawn_omega=false`, WM stock-carrier use of `697` / `49126` remains retired while normal Summon Voidwalker `697` is preserved, and the native module builds with low physical bleed ticks plus a 7.5% Alpha echo melee-hook behavior capped at `40` active Echoes
