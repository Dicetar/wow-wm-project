...
- managed item reward proof is `WORKING` at DB/runtime-reload level: `control/examples/items/night_watchers_lens.json` publishes item `910006` (`Night Watcher's Lens`) from a reserved WM item slot, and BridgeLab quest `910024` (`Bounty: Nightbane Dark Runner - Lens`) now rewards it through `RewardItem1=910006`, `RewardAmount1=1`, plus the existing 12 silver; the current lens effect uses visible aura spell `132` as the client-known marker and `mod-wm-spells` restores mana on player/pet/owned-summon kills only while the item is equipped and the marker aura is present; client-visible equip/passive proof is still `PARTIAL` until the fresh quest is turned in and the item is equipped in-game
