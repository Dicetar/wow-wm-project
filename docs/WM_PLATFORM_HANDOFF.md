...
- spell pipeline V1 is `WORKING` at repo-test level: `python -m wm.spells.publish` and `python -m wm.spells.rollback` now cover helper/proc-table preflight, rollback snapshots, deterministic SQL planning, clear-vs-restore reserved-slot status, and structured schema/snapshot/MySQL failures; `control/examples/spells/defias_pursuit_instinct.json` is the current bundled example, and BridgeLab live publish/learn/rollback proof remains `PARTIAL`
