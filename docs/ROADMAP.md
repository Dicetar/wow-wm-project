...
- spell pipeline V1 is `WORKING` at repo-test level through `python -m wm.spells.publish` and `python -m wm.spells.rollback`: helper/proc table preflight, snapshot preview, deterministic SQL planning, rollback clear-vs-restore slot handling, and structured snapshot/schema/MySQL failures are covered; BridgeLab live publish/learn/rollback proof is still `PARTIAL`
