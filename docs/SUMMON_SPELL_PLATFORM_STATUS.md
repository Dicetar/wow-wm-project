...
- named shell entries exist for `940000`, `940001`, `944000`, `945000`, and `946600`
...
- `WORKING` repo/build/deploy on 2026-04-27: Echo seek target selection now uses the active WM player as the search center, so `wm echo seek [yards]` prioritizes the nearest eligible hostile to Jecia rather than each Echo's own nearest hostile. Echo Restorer filler DPS now starts a real visible Mind Blast `8092` cast instead of using a private fake cast timer followed by a triggered hit; in seek mode, Restorers can leave the close support ring and move toward the selected hostile until the stock visible cast can fire. Runtime name changes now force object-visibility refresh so active Echoes can update to `Echo Destroyer` / `Echo Restorer`. Bonebound Echo Stasis now adds active Echo Destroyer/Echo Restorer counts into the saved pool instead of replacing it, restores only when no Echoes are active, and preserves any over-cap remainder in the pool. Echo follow positioning now uses deterministic formation rings with a 1.6 yard minimum spacing target instead of random follow slots, so large Echo groups spread into outer rings instead of model-merging. BridgeLab worldserver was restarted to pid `28000`; live in-game proof remains `PARTIAL`.
...
- `PARTIAL`: rare support Echo variant is repo-tested, SQL-applied, native-built, and deployed to BridgeLab worldserver pid `32420`: Alpha melee has a separate `5%` support-role proc chance that spawns creature `920103` (`Echo Restorer`) with Skeletal Magelord display `11397` from stock NPC `15121` and one random configured level-appropriate rare/epic staff model, copies Alpha health/stats through the same owner-intellect stat transfer path, matches Echo Destroyer template walk/run speed and runtime movement rates after stat recalculation, heals the lowest hurt owner/Alpha/Echo/group target with visible Flash Heal `2061`, applies visible Renew `139`, applies visible Power Word: Shield `17` only to support targets under active attack/cast threat, and never uses the melee attack path. Restorers use a separate active cap of `10`, repeated Echo Destroyer spawns without a successful Restorer force a support spawn attempt after `6` Destroyer spawns, and spell output adds owner shadow spell power through `priest_echo_spell_power_to_healing_pct=35`, `priest_echo_spell_power_to_shield_pct=30`, and `priest_echo_spell_power_to_damage_pct=45`. Filler DPS is now a visible Mind Blast hit using spell `8092` as both effect and damage spell, with `priest_echo_dps_cast_time_ms=1500`, `priest_echo_dps_damage_pct=19`, `priest_echo_dps_cooldown_ms=2500`, and `priest_echo_dps_max_range=100.0`. Restorers now use a randomized close support ring around `priest_echo_safe_follow_distance=1.8`, intended to place them between Jecia and the wider Echo Destroyer ring instead of stacking on one point. Single disease/curse dispel is available through visible spells `528`/`475`; Mass Dispel `32375` is gated by severity/affected-target thresholds and a `180000 ms` cooldown. Live in-game proof of the retuned filler/positioning/speed/seek is still pending.
- `PARTIAL`: Alpha Echo attack/reacquire and seek-mode control are repo-tested, native-built, and deployed to BridgeLab worldserver pid `32420`: maintenance now explicitly adds threat, sets combat on Echo and target, calls `AttackStart`, falls back to direct `Attack`, and forces `MoveChase` when Echo Destroyers have Alpha's victim but follow motion has stuck. Player-facing chat control now exists for the active WM player: type `wm echo seek` to make Echo Destroyers and ranged Echo Restorers select the nearest eligible hostile in range when Alpha has no current victim, `wm echo seek 60` to enable seek at a specific radius, `wm echo range 60` to retune seek radius without changing mode, `wm echo follow` to return them to close guard behavior, and `wm echo teleport` / `wm echo tp` / `wm echo recall` to teleport all active Echoes back to the player. Runtime seek radius is per-player, scoped to the active WM player, and clamped to `5-100` yards. In-game proof is still pending.
