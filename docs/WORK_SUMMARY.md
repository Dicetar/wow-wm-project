...
- Primitive Pack 2 is `PARTIAL`: `player_cast_spell`, `player_set_display_id`, `creature_cast_spell`, `creature_set_display_id`, and `creature_set_scale` now have typed native bodies, payload contracts, disabled policy seed SQL, repo tests, and a successful BridgeLab native build; live control apply is still blocked until player `5406` is online, because request `83` failed with `player_not_online`
...
- Primitive Pack 2 is not a default gameplay surface yet. Its cast/display/scale verbs are implemented and built, but live in-game proof is `PARTIAL`; keep them policy-disabled outside scoped BridgeLab operator tests.
