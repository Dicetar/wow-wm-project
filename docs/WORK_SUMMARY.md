...
- spell pipeline V1 is now `WORKING` at repo-test level: `python -m wm.spells.publish` discovers live `spell_linked_spell` / `spell_proc` columns, snapshots current helper/proc rows, and builds deterministic publish SQL, while `python -m wm.spells.rollback` restores or clears those rows, keeps reserved spell slots `active` when previous rows existed, returns them to `staged` when the slot was previously empty, and fails explicitly on malformed snapshots or unsupported live restore schemas; BridgeLab live spell proof is still `PARTIAL`
