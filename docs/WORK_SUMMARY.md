...
- `scripts/bridge_lab/Start-BridgeLabAutoBounty.ps1` is now the repo-owned BridgeLab start lane for the dynamic path: it stops the current native watcher, clears existing `reactive_bounty:*` rows for the scoped player by default, then restarts `wm.events.watch` with auto-bounty enabled and armed from the end
