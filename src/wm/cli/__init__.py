...
This package is the canonical home of the WM command-line surface. It exposes
the unified dispatcher (`main`) and the discovery `CATALOG` directly, and holds
the individual CLI modules as submodules (e.g. `wm.cli.slice_demo`).

`main` dispatches `wm <command>` to any importable `wm.*` module via runpy, so
the catalog drifting out of date degrades discovery only, never execution.
"""
