...
def build_quest_candidates_v2(path: str | Path, limit: int = 5) -> CandidateSet:
    rows, schema_names = _load_rows_and_schema(path)
    options = _build_candidates_v2(
        rows,
...
def build_item_candidates_v2(path: str | Path, limit: int = 5) -> CandidateSet:
    rows, schema_names = _load_rows_and_schema(path)
    options = _build_candidates_v2(
        rows,
        schema_names=schema_names,
...
def build_spell_candidates_v2(path: str | Path, limit: int = 5) -> CandidateSet:
    rows, schema_names = _load_rows_and_schema(path)
    options = _build_candidates_v2(
        rows,
        schema_names=schema_names,
        ...
        preferred_summary_keys=["School", "Category", "ManaCost", "ManaCostPercentage", "RangeIndex", "DurationIndex", "Effect1", "Effect2", "Effect3"],
        limit=limit,
    )
...
def _load_rows_and_schema(path: str | Path) -> tuple[list[dict[str, Any]], list[str]]:
    ...
    if isinstance(loaded, ExportBundle):
        ...
        schema_names = [str(x.get("name")) for x in loaded.schema if isinstance(x, dict) and x.get("name")]
        return rows, schema_names
    return [], []


def _build_candidates_v2(
    rows: list[dict[str, Any]],
    *,
