...
def build_quest_candidates_v3(path: str | Path, limit: int = 5) -> CandidateSet:
    rows, schema_names = _load_rows_and_schema(path)
    options = _build_candidates_v3(
        rows,
...
def build_item_candidates_v3(path: str | Path, limit: int = 5) -> CandidateSet:
    rows, schema_names = _load_rows_and_schema(path)
    options = _build_candidates_v3(
        rows,
        schema_names=schema_names,
...
def build_spell_candidates_v3(path: str | Path, limit: int = 5) -> CandidateSet:
    rows, schema_names = _load_rows_and_schema(path)
    options = _build_candidates_v3(
        rows,
        schema_names=schema_names,
