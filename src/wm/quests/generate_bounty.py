...
def _build_parser()->argparse.ArgumentParser:
    p=argparse.ArgumentParser(prog="python -m wm.quests.generate_bounty"); qg=p.add_mutually_exclusive_group(required=True); qg.add_argument("--questgiver-entry",type=int); qg.add_argument("--questgiver-name"); tg=p.add_mutually_exclusive_group(required=True); tg.add_argument("--target-entry",type=int); tg.add_argument("--target-name"); p.add_argument("--quest-id",type=int,required=True); p.add_argument("--kill-count",type=int,default=8); p.add_argument("--reward-money-copper",type=int,default=1200); p.add_argument("--output-json",type=Path,default=Path("artifacts/generated_bounty.json")); p.add_argument("--summary",action="store_true"); return p

def _render_summary(payload:dict[str,Any], output_path:Path)->str:
    d=payload.get("draft",{}); o=d.get("objective",{}); td=d.get("template_defaults",{}); return "\n".join([f"quest_id: {d.get('quest_id')}",f"title: {d.get('title')}",f"questgiver_entry: {d.get('questgiver_entry')} | {d.get('questgiver_name')}",f"target_entry: {o.get('target_entry')} | {o.get('target_name')}",f"kill_count: {o.get('kill_count')}",f"template_defaults: {', '.join(sorted(td.keys())) if td else '(none)'}",f"output_json: {output_path}"])
