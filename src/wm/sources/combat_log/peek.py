...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Read-only peek into the raw combat-log source.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    cursor = store.get_cursor(adapter_name="combat_log", cursor_key="state")
    scanner = CombatLogScanner(client=client, settings=settings, reactive_store=reactive_store)
    result = scanner.scan(
...
def _emit_output(*, payload: dict[str, object], summary: bool, output_json: Path | None) -> None:
    ...
    if summary:
        ...
        for index, signal in enumerate(signals, start=1):
            ...
            print(
                f"signal[{index}] at={signal.get('occurred_at')} player={player.get('name') or player.get('guid')} "
                f"subject={subject.get('name') or subject.get('entry')} offset={signal.get('byte_offset')} "
                f"resolution={signal.get('resolution_source')}"
            )
        for index, failure in enumerate(failures, start=1):
            ...
            print(
                ...
                f"event={failure.get('event_name')}"
            )
    else:
        print(raw)
