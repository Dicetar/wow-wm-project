...
class LiveTargetResolver:
    ...
    def resolve_creature_entry(self, entry: int) -> LiveTargetProfile | None:
        rows = self.client.query(
            ...
            sql=_build_live_target_sql(int(entry)),
        )
        if not rows:
            return None
...
def _build_live_target_sql(entry: int) -> str:
    return f"""
SELECT
    ct.entry,
    ct.name,
    ...
    (
        SELECT GROUP_CONCAT(CONCAT(c.map, ':', c.zoneId, ':', grouped.spawn_count) ORDER BY grouped.spawn_count DESC SEPARATOR ';')
        FROM (
        ...
        JOIN creature c ON c.id1 = ct.entry AND c.map = grouped.map AND c.zoneId = grouped.zoneId
        GROUP BY ct.entry
    ) AS spawn_contexts
